...
class handDetector():
    ...
    def findPosition(self, img, handNo=0, draw=True):
        lmList = []
        img, results = self.findHands(img)  # Obtenir les résultats de la méthode findHands
        if results.multi_hand_landmarks:
            myhand = results.multi_hand_landmarks[handNo]  # Utilisation des résultats obtenus
